(function () {
  ...
  function buildCave() {
    ...
    // mobile gets its own canvas scale, grid, node size and clearance —
    // a smaller viewport needs a proportionally bigger multiplier and
    // finer-grained zones for the spread to actually read as spread
    const GRID_COLS = isTouchPrimary ? 4 : 6;
    const GRID_ROWS = isTouchPrimary ? 5 : 3; // 4x5 = 20 zones (18 used, 2 empty) on mobile
    CAVE_WIDTH = Math.round(vw * (isTouchPrimary ? 5 : 4));
    CAVE_HEIGHT = Math.round(vh * (isTouchPrimary ? 4 : 3));

    caveContent.style.width = CAVE_WIDTH + "px";
    caveContent.style.height = CAVE_HEIGHT + "px";

    // node footprint — matches the CSS width for .work-node/.frame at
    // this breakpoint (see the `(pointer: coarse)` rule in style.css)
    const NODE_W = isTouchPrimary ? 160 : 220;
    const NODE_H = isTouchPrimary ? 250 : 320;
    const PAD = 20; // minimum clearance from the canvas edge
    const MARGIN = isTouchPrimary ? 20 : 30; // minimum clearance from a
    // zone's own edge, so two nodes in adjacent zones both pushed toward
    // their shared border are still guaranteed MARGIN + MARGIN apart —
    // 40px on mobile, 60px on desktop
    const JITTER = isTouchPrimary ? 20 : 30; // max random offset from zone center, in px
  ...
  const Heartbeat = {
    ...
    // must be called synchronously from within the "enter ?" click
    // handler — never before, never in a setTimeout. iOS Safari only
    // grants audio output to an AudioContext that is both created AND
    // resumed inside the actual gesture's call stack; creating it later
    // (even a tick later) leaves it permanently silent on iOS.
    unlock() {
      if (this.ctx) return;
    ...
    // begins the actual lub-dub scheduling. Safe to call after a delay —
    // the context itself was already created + resumed inside the gesture
    // by unlock(), so starting playback later doesn't re-trigger autoplay
    // restrictions, it just uses the already-unlocked context.
    start() {
  ...
  // touch is the primary pointer — no mouse, no hover; drives every
  // mobile-specific branch below (torch size, panning source, hint copy)
  ...
  let baseRadius = isTouchPrimary ? 220 : 170;
  ...
  // On touch devices the same lerp is reused, but the drive signal is
  // gyroscope tilt instead — the torch stays put at screen center and
  // the world tilts around it (with a plain touch-drag fallback, which
  // needs no extra code: it's native scroll on #cave-viewport).
  ...
  function handleOrientation(e) {
    ...
    if (!gyroBaseline) {
      gyroBaseline = { gamma: e.gamma, beta: e.beta };
    }
    gyroActive = true;
    const dGamma = e.gamma - gyroBaseline.gamma; // left/right tilt
    const dBeta = e.beta - gyroBaseline.beta; // forward/back tilt
    const SENS = 16; // px of pan per degree of tilt from the calibrated baseline
    const MAX_TILT_PAN = 280;
    gyroTargetX = Math.max(-MAX_TILT_PAN, Math.min(MAX_TILT_PAN, -dGamma * SENS));
    gyroTargetY = Math.max(-MAX_TILT_PAN, Math.min(MAX_TILT_PAN, -dBeta * SENS));
  }

  // must be called from within a user gesture on iOS 13+, which is why
  // this is invoked from the "enter." click handler, not on page load
  function enableGyro() {
    ...
    if (typeof DeviceOrientationEvent.requestPermission === "function") {
      DeviceOrientationEvent.requestPermission()
        .then((result) => {
          ...
          // denied — silently fall back to touch-drag (native scroll)
        })
        .catch(() => {
          /* not supported / dismissed — fall back to touch-drag */
        });
    } else {
      // Android and other browsers that don't gate orientation behind a prompt
      window.addEventListener("deviceorientation", handleOrientation);
    }
  }
  ...
  function drawFrame(now) {
    ...
    // on touch, the torch stays put at screen center — the world moves
    // around it instead (gyro tilt or a plain drag), which is what makes
    // exploring by hand feel natural instead of chasing a moving light
    let px, py;
    if (isTouchPrimary) {
      px = w / 2;
      py = h / 2;
    ...
    // On touch with gyro active, tilt drives the target instead; with no
    // gyro, this settles to (0,0) and touch-drag (native scroll) takes over.
    if (caveContent) {
      let targetX, targetY;
      if (isTouchPrimary && gyroActive) {
        targetX = gyroTargetX;
        targetY = gyroTargetY;
      } else {
        targetX = -(px - w / 2) * PARALLAX_FACTOR;
        targetY = -(py - h / 2) * PARALLAX_FACTOR;
      }
  ...
  function startTorch() {
    ...
    // no touch listeners for torch position — on touch devices the torch
    // stays fixed at screen center (see drawFrame) while the cave itself
    // moves via gyro tilt or native drag-scroll on #cave-viewport
  ...
  function showCaveHintOnce() {
    ...
    if (typeof i18n !== "undefined") {
      caveHint.textContent = i18n.t(isTouchPrimary ? "move_to_explore_mobile" : "move_to_explore");
    }
  ...
  function enterCave() {
    ...
    // create + resume the AudioContext synchronously, right here in the
    // click handler — this is the only place iOS Safari will allow it.
    // The lub-dub scheduling itself still waits for the cave to become
    // visible (see startCavePhase / Heartbeat.start), but the context
    // itself must be born inside this exact gesture or it stays silent.
    Heartbeat.unlock();

    // request gyroscope access now — this click is the user gesture
    // iOS requires for DeviceOrientationEvent.requestPermission()
    enableGyro();

    // heartbeat playback is intentionally NOT started here — it must
    // stay silent through the entire entry screen and tear, and only
    // begin once the cave itself is visible. See startCavePhase().
